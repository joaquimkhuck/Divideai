...
export default function CreditosSucesso() {
  ...
  return (
    <PhoneShell className="px-6 pb-8 pt-14">
        ...
        {isLoading ? (
          ...
          <>
            ...
            <p className="mt-3 max-w-xs text-[17px] text-muted-foreground">
              Volte para Créditos e confira seu saldo.
            </p>
          </>
        ) : (
          <>
            ...
            <p className="mt-3 max-w-xs text-[17px] text-muted-foreground">
              {pago
                ? `${data.credits} ${data.credits === 1 ? "crédito adicionado" : "créditos adicionados"} à sua conta.`
            ...
            <Card className="mt-8 w-full p-4 text-left">
              ...
              <p className="mt-2 font-bold" data-testid="text-saldo-atualizado">
                {data.creditBalance} {data.creditBalance === 1 ? "conta" : "contas"}
              </p>
            </Card>
          </>
        )}
      ...
      <Button size="lg" onClick={() => setLocation("/creditos")}>
        Voltar aos créditos
      </Button>
    </PhoneShell>
  );
